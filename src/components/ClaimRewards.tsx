...
export function ClaimRewards() {
  const { rounds, prizeConfig } = useGame()
  const { user } = useAuth()
  ...
  const isWinner = useMemo(() => {
    if (!latestFinished || !user) return false
    return latestFinished.winningAddress && user.address && latestFinished.winningAddress.toLowerCase() === user.address.toLowerCase()
  }, [latestFinished, user])
    ...
    <Card className="glass-card border-2 border-emerald-500/40">
      ...
      <CardContent className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <Button disabled className="bg-gray-700">Claim 2nd</Button>
          <Button disabled className="bg-gray-700">Claim Jackpot</Button>
        </div>
        {!isWinner && (
