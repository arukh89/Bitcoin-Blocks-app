...
export async function GET(request: Request): Promise<Response> {
  const BASE = process.env.MEMPOOL_API_BASE
  if (!BASE) {
    return NextResponse.json({ error: 'Server misconfiguration: MEMPOOL_API_BASE is not set' }, { status: 500 })
  }
      ...
      while (attempts < maxAttempts) {
        try {
          ...
          const response = await fetch(
            `${BASE}/v1/mining/blocks/timestamp/${timestamp}`,
            {
              method: 'GET',
