...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function initConnection(): Promise<void> {
      try {
        ...
        const conn: any = await connectToSpacetime({
          onConnect: () => {
            if (!mounted) return
            setConnected(true)
          },
  ...
  useEffect(() => {
    ...
    // Load initial data
    const initialRounds = (Array.from(((client as any).db as any).rounds.iter()) as STDBRound[]).map(convertRound)
  ...
  useEffect(() => {
    ...
    // Load initial data
    const initialGuesses = (Array.from(((client as any).db as any).guesses.iter()) as STDBGuess[]).map(convertGuess)
  ...
  useEffect(() => {
    ...
    // Load initial data
    const initialLogs = (Array.from(((client as any).db as any).logs.iter()) as STDBLog[]).map(convertLog)
    setLogs(initialLogs)
  ...
  useEffect(() => {
    ...
    // Load initial data
    const initialChat = (Array.from(((client as any).db as any).chatMessages.iter()) as STDBChatMsg[]).map(convertChatMsg)
  ...
  useEffect(() => {
    ...
    // Load initial data
    const initialConfigs = (Array.from(((client as any).db as any).prizeConfig.iter()) as STDBPrizeConfig[]).map(convertPrizeConfig)
